...
- **密鑰版本控制** – 加密密鑰版本管理（V1/V2），自動遷移舊版本
- **智能密碼緩存** – 15 分鐘緩存，登出、關閉標籤頁或不活動時自動清除
- **網絡數據隔離** – 本地記錄標記網絡（testnet/mainnet/local）防止混淆
- **Seal 權限模型** – 公開/私密記錄分離，智能解密
- **訪問控制標籤** – 授權地址支持自定義標籤（如「伴侶」、「家人」），同步到雲端
...
- **功能：** 
  - 完全本地存儲、AES-GCM 256 加密、自動密鑰管理
  - 設置頁面訪問（⚙️）用於密碼管理和數據遷移
  - 語言/主題切換器（無網絡切換器，因為是純本地模式）

### 時間線體驗

**功能：**
- 搜索、過濾（標籤、日期、儲存類型）和排序
- **網絡徽章** – 視覺化顯示記錄來源（🧪 測試網 / 🌐 主網 / 💻 本地）
- 批量操作（導出/刪除）
...
**Seal Access Policies** – 在鏈上授權和撤銷私密 NFT 記錄的訪問權限。

**功能：**
- **授權地址** – 授權特定錢包地址訪問您的私密記錄
- **自定義標籤** – 為授權地址添加角色標籤（例如「伴侶」、「家人」、「治療師」、「醫生」、「AI 助手」）
- **雲端同步** – 標籤同步到雲端（Supabase），可跨設備訪問
- **鏈上驗證** – 所有授權操作記錄在 Sui 區塊鏈上
- **隨時撤銷** – 立即撤銷訪問權限，即時生效
- **訪問歷史** – 查看所有授權/撤銷操作的完整審計記錄
- **公開記錄** – 公開記錄任何人都可訪問（無需授權）

**使用方法：**
1. 將情感記錄鑄造為 NFT，啟用「Seal Access Policies」選項
2. 在時間線中，點擊記錄卡片上的「訪問控制」
3. 輸入錢包地址和可選的角色標籤
4. 授權訪問（鏈上交易）
5. 被授權地址現在可以解密並查看您的私密記錄

**標籤管理：**
- 授權時添加標籤或稍後編輯
- 標籤存儲在本地並同步到雲端，支持多設備訪問
- 撤銷訪問時標籤會自動刪除

## 🔑 密碼管理與設置

Echoma 提供完善的密碼管理和安全設置功能。
...
- **密碼強度指示器** – 實時視覺反饋，彩色進度條顯示密碼強度（弱/中等/強）
- **首次設置** – 首次使用時可選的密碼設置對話框（可跳過）
- **密碼重置** – 更改密碼並自動重新加密所有數據
- **備份提醒** – 密碼重置或數據遷移前的重要警告提示
- **智能緩存** – 密碼緩存 15 分鐘，登出或關閉頁面時自動清除
- **密碼提示** – 可選的提示信息儲存在本地（明文），幫助您記憶
- **密鑰版本控制** – 自動版本追蹤（V1：派生密鑰，V2：密碼密鑰）
...
- 您的密碼**不會被儲存或上傳**到任何地方
- 如果忘記密碼，您**無法**解密您的數據
- **在密碼重置或遷移前務必備份數據**（導出為 CSV/JSON 格式）
- 建議使用密碼管理器或記下密碼提示
- 密碼僅適用於本地/MVP 模式數據（不適用於 Walrus 儲存的數據）

📖 **數據庫遷移：**
- 某些功能需要更新數據庫架構（例如訪問控制標籤的雲端同步）
- 詳見 [DATABASE_MIGRATION_GUIDE.md](./DATABASE_MIGRATION_GUIDE.md) 獲取詳細步驟
- 可通過 Supabase CLI 或 Dashboard 應用遷移
...
- 用戶自定義加密密碼，支持密鑰版本控制（V1/V2）
- **密碼強度指示器**，實時視覺反饋（弱/中等/強）
- **備份提醒**，關鍵操作前的安全警告（密碼重置、數據遷移）
- 設置頁面，包含密碼管理、數據遷移和緩存控制
- 訪問控制標籤，支持雲端同步跨設備訪問
- 智能密碼緩存，自動清理（登出、關閉標籤頁、不活動）
- 網絡數據隔離，帶**視覺徽章**（testnet/mainnet/local）
- **MVP 模式增強**：設置頁面訪問、語言/主題切換器
- 數據庫遷移指南，用於 Supabase 架構更新
